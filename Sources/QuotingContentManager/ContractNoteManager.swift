...
public struct ContractNoteManager: Sendable {
    ...
    public func fetchNotes(serviceItemConfig key: String, value: String) -> [ContractNoteInfo] {
        let tag = "ServiceItemConfig/\(key):\(value)"
        return fetchNotes(subsetOf: [tag])
    }
    ...
    public func findIndexToInsert(willInsert noteInfo: ContractNoteInfo, inUniqueCodes uniqueCodes: [String?]) -> Int? {
        uniqueCodes.firstIndex(where: { uniqueCode in
            guard let currentNoteInfo = notes.first(where: { $0.uniqueCode == uniqueCode }) else {
                return false
            }
